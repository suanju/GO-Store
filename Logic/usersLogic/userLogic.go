...
func Register(data *userModel.RegisterReceiveStruct) (results interface{}, err error) {
	...
	//判断验证码是否正确
	verCode, err := Redis.Db.Get(fmt.Sprintf("%s%s", Redis.KeyName.RegEmailVerCode, data.Email)).Result()
	if err == redis.Nil {
...
func SendEmailVerCode(data *userModel.SendEmailVerCodeReceiveStruct) (results interface{}, err error) {
	...
	err = Redis.Db.Set(fmt.Sprintf("%s%s", Redis.KeyName.RegEmailVerCode, data.Email), code, 5*time.Minute).Err()
	if err != nil {
		return nil, err
	}
	return "发送成功", nil
...
func SendEmailVerCodeByForget(data *userModel.SendEmailVerCodeReceiveStruct) (results interface{}, err error) {
	...
	err = Redis.Db.Set(fmt.Sprintf("%s%s", Redis.KeyName.RegEmailVerCodeByForget, data.Email), code, 5*time.Minute).Err()
	if err != nil {
		return nil, err
	}
	return "发送成功", nil
...
func Forget(data *userModel.ForgetReceiveStruct) (results interface{}, err error) {
	...
	//判断验证码是否正确
	verCode, err := Redis.Db.Get(fmt.Sprintf("%s%s", Redis.KeyName.RegEmailVerCodeByForget, data.Email)).Result()
	if err == redis.Nil {
		return nil, fmt.Errorf("验证码过期！")
	}
